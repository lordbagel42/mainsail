    ...
    <div>
        <div v-for="(board, index) of boardsWithAxes" :key="board.key">
            ...
            <div class="px-6">
                ...
                <odrive-axis-tuning v-for="axis in board.axes" :key="axis.key" :axis="axis" class="mb-3" />
            </div>
        </div>
    </div>
</template>
...
import OdriveAxisTuning from '@/components/panels/Odrive/OdriveAxisTuning.vue'
import type { PrinterStateOdrive } from '@/store/printer/types'

@Component({
    components: { OdriveAxisTuning },
})
export default class OdriveTuningList extends Mixins(BaseMixin) {
